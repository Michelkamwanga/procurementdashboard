...
if uploaded_file:
    ...
    st.markdown("""<style>.metric-container {background-color: orange;border: 2px solid green;border-radius: 10px;padding: 15px;text-align: center;font-weight: bold;color: white;font-size: 15px;}.metric-value {font-size: 30px;font-weight: bold;}</style>""", unsafe_allow_html=True)
    
    # Fonction pour afficher les métriques avec style
    def display_metric(label, value, col):
        col.markdown(f"""<div class='metric-container'><div class='metric-value'>{value}</div><div>{label}</div></div>""", unsafe_allow_html=True)
    ...
    with st.container():
        ...
        fig2.update_layout(
            ...
            legend=dict(
                ...
                traceorder='normal',  # Ordre des éléments de la légende
                font=dict(size=14),  # Taille de la police de la légende
                bgcolor='rgba(215, 285, 255, 0.5)',  # Couleur de fond de la lég
